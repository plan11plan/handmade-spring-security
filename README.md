...
## 2️⃣ 인증 아키텍처 구현하기
###  ✅ 구현 완료 조건
- [ ] AuthenticationManager를 정의하고 구현한다.
- [ ] 여러 인증 방식을 지원한다.
- [ ] 인증 실패를 적절히 처리한다.
- [ ] Filter 기반 인증을 구현한다.
...
- [ ] 인증 API를 정의한다.
  - authenticate() 메서드 정의
  - 인증 성공 -> 완전히 채워진 Authentication 반환
  - 인증 실패 -> AuthenticationException 발생

### 📌 ProviderManager
- [ ] AuthenticationManager를 구현한다.
  - AuthenticationProvider 목록 관리
  - 각 Provider에게 순차적으로 인증 위임
  - 하나라도 성공하면 즉시 반환
  - 모두 실패하면 예외 발생
- [ ] 부모 AuthenticationManager를 지원한다.
  - 선택적 부모 설정
  - 모든 Provider 실패 시 부모에게 위임
